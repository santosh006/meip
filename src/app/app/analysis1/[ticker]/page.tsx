...
export default async function AnalysisPage({ params }: AnalysisPageProps) {
  ...
  // impact_records has no entity_id foreign key, so it is matched by company.
  ...
  return (
    <main className="max-w-5xl mx-auto px-4 py-8 space-y-8">
      <header className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <p className="text-sm text-gray-500">{entity.ticker ?? ticker}</p>
      </header>
      ...
      <section>
        ...
        {recordsError ? (
          <p className="text-sm text-red-600">
            Failed to load impact records: {recordsError.message}
          </p>
        ) : !impactRecords || impactRecords.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            No impact records found for this entity.
          </p>
        ) : (
                ...
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div>
                    <h3 className="font-semibold text-gray-900">
                      {record.event_type ?? "Impact event"}
                    </h3>
